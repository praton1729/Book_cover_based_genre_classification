...
\author{\IEEEauthorblockN{1\textsuperscript{st} Prathu Baronia}
\textit{Indian Institute of Technology, Bombay}\\
India
}
\and
\IEEEauthorblockN{2\textsuperscript{nd} Balraj Parmar}
\textit{Indian Institute of Technology, Bombay}\\
India
...
Don’t judge a book by it’s cover is a common phrase in English which tells us not to evaluate a book from it’s cover page only. In this project we aim to do exactly this and show that it is possible to predict the genre of a book from the cover page art and the title of the book.
Transfer Learning approach is used to extract features from the dataset using VGG-16 Convolutional Neural Network which was pretrained on the Imagenet Dataset. A simple neural network of three layers is trained using this features as input with a validation accuracy of 72.3\%
...
\bibitem{b8} \href{https://en.wikipedia.org/wiki/Bag-of-words_model}{Bag of Words Wiki}
\end{thebibliography}
